...
void main() {
    ...
    // Base interference (always present)
    ...
    // Momentum-based ripple interference
    ...
    // Base horizontal distortion (always present)
    float base_horizontal_distortion = (
        sin(uv.y * 2.0 + iTime * 1.0) + 
        sin(uv.y * 50.0 + iTime * 5.7) * 0.3 + 
        sin(uv.y * 500.0 + iTime * 20.0) * 0.1) * horizontal_distort_distance * base_interference;
    
    // Ripple-based horizontal distortion
    ...
    // Base vertical distortion (always present)
    float base_vertical_distortion = sin(uv.y * 2.5 + 5.1 + iTime * 1.4) * 
        sign(sin(uv.y * 3.6 + iTime * 2.4)) * vertical_scroll_distance * base_interference;
    
    // Ripple-based vertical distortion
    ...
    // Base scatter effect (always present)
    vec2 base_scatter = vec2(noise(uv + iTime), 0.0) * 
        max(0.0, base_interference - 0.5) * 0.1;
    
    // Ripple-based scatter effect
    vec2 ripple_scatter = vec2(noise(uv + iTime + mouseUV), 0.0) * 
        rippleAmplitude * 0.1;
    
    vec2 scatter = base_scatter + ripple_scatter;
    
    // Base chromatic aberration (always present)
    float base_aberration = constant_chromatic_aberration + 
        distort_chromatic_aberration * (0.1 + base_interference);
    
    // Momentum-enhanced aberration
    float momentum_aberration = distort_chromatic_aberration * rippleAmplitude * 0.3;
    
    float aberration_strength = base_aberration + momentum_aberration;
    
    // Base noise (always present)
    float base_noise_alpha = (constant_noise * interference + 
        base_interference * scrolling_noise * 0.3) *
        sin(iTime * 23.4 + noise(rounded_uv) * 123.4);
    
    // Momentum-enhanced noise
    ...
    // Sample the background texture with mouse-influenced chromatic aberration
    vec3 image_color = min(noise_alpha * noise_color.rgb + vec3(
        texture(u_texture0, image_uv - vec2(aberration_strength, 0.0)).r,
        texture(u_texture0, image_uv).g,
        texture(u_texture0, image_uv + vec2(aberration_strength, 0.0)).b), 1.0) - scanline;
